...
select ibd.bidder_id, NO_OF_BIDS as Total_Bids, count(bid_status) as Bids_Won, 
(count(BID_STATUS)/NO_OF_BIDS)*100 as Win_Percentage
...
select s.stadium_id, stadium_name, city, sum(if(toss_winner=match_winner,1,0)) / 
sum(if(toss_winner=match_winner,1,1)) * 100 as P
...
select Match_id, (select team_name from ipl_team it where if(im.match_winner = 1, it.team_id = 
im.team_id1, it.team_id = im.team_id2)) as 'Match Winner', 
 if(match_winner =1, team_id1, team_id2) as Winning_Team_ID,
 (select team_name from ipl_team it where if(im.match_winner = 2, it.team_id = im.team_id1, 
it.team_id = im.team_id2)) as 'Match against', win_details
from ipl_match im;

## 6. Display total matches played, total matches won and total matches lost by team along with  its team name.
select its.TEAM_ID, TEAM_NAME, sum(MATCHES_PLAYED)as 'Matches Played', sum(MATCHES_WON)
as 'Matches Won', sum(MATCHES_LOST) as 'Matcehs Lost', sum(TOTAL_POINTS) as "Points"
